...
public class ReceiveDemo {
	...
	static class MyMessageListener implements MessageListener{
		...
		public boolean dealMessage(Message message) {
            ...
            try{
            	...
            	String content=(String)contentMap.get("content");//短信内容
            	//System.out.println("$$$"+content+"$$$");
            	String phoneNumber=(String)contentMap.get("phone_number");//手机号码
            	Date today = new Date();
            	String sendTime=format.format(today);//发送日期
            	String sql="select id from leave_user where user_phone=?";//根据手机号码的唯一性确定其id
            	List<List> listid = JDBC.list(sql, phoneNumber);
            	System.out.println(listid);
            	int id = (Integer)listid.get(1).get(0);
            	sql = "insert into temporary_leave(user_id,content,message_date) value(?,?,?)";
            	boolean b = JDBC.executeUpdate(sql, id,content,sendTime);
            	if(b) System.out.println("临时请假信息插入成功");
            	else System.out.println("临时请假信息插入失败");
	...
	static DefaultAlicomMessagePuller puller=null;
	public static void start() throws Exception, ParseException {
		...
		String accessKeyId=Config.get("db.accessKeyId");
		String accessKeySecret=Config.get("db.accessKeySecret");;
		...
		*/
		...
		String queueName=Config.get("db.queueName");//在云通信页面开通相应业务消息后，就能在页面上获得对应的queueName,格式类似Alicom-Queue-xxxxxx-SmsReport
